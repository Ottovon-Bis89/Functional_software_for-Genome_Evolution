...
class Node:
    ...
    def get_operations(self,source_genome, target_genome):
        ...
        while switch:  # Start a while loop that continues until the switch is turned off
            ...
            if count_applicable_region > 0:
                ...
                elif foreign_Dna_counter <= 3 or foreign_Dna_counter != 0:  # Check if the 'foreign_Dna_counter' is greater than  or equal to 0
    ...
    def mutation_legal_operations(self, source_genome, target_genome):
        """
        This function decides if mutations can take genome A to genome B.
        ...
        for j in range(len(target_genome)): # Iterate over each chromosome in the target genome
            ...
            # Check if the gene is not present in the target chromosome or present but at different index in the source chromosome
                ...
                if gene not in target_chromosome:
                    deletion_target.append((index, gene))

                # elif index != target_chromosome.index(gene):
                #     deletion_target.append((index, gene))


            # for index, gene in enumerate(new_chromosome_with_positions_source):
            #     if gene not in target_chromosome:
            #         deletion_target.append((index, gene))
                # else:
                #     target_index =target_chromosome.index(gene)
        ...
        if len(target_genome) > len(new_source_genome_with_positions):
            for gene_target in range(len(new_source_genome_with_positions), len(target_genome)):
                ...
                # print("howwwwwwwwwwwwwwwwwwwwwwww")
                deletion_genome.append(deletion_target)  # Append the deletion_target to the deletion_genome list
    ...
    def do_mutation(self, source_genome, required_mutation):
        ...
        """
            ...
            Args:
                source_genome (list): A list representing the source genome with approved intergenic regions.
                required_mutation (list): A list representing the required mutation.

            Returns:
                tuple: A tuple containing the updated source_genome and a list of mutation records.
        """
        ...
        if mutation_type == 'dup':
            ...
            # If the condition is true, perform the duplication operation
                ...
                operation = {'Mut_Type': mutation_type, 'Chr': chromosome_index + 1,'Gene': gene_to_duplicate,'Pos': position,'Type of dup': type_of_duplication,'Genome after mutation': source_genome}
            ...
            if position > len(chromosome)-1 or '_' in str(chromosome[position]):
                ...
                operation = {'Mut_Type': mutation_type,'Chr': chromosome_index + 1,'Pos': len(chromosome)+1,'Gene': gene_to_insert,'Genome after mutation': source_genome}
               ...
               # Create a record to keep track of the mutation operation
                operation = {'Mut_Type': mutation_type,'Chr': chromosome_index + 1,'Pos': position,'Gene': gene_to_insert,'Genome after mutation': source_genome}
            ...
            if(isinstance(chromosome[position-1], str) and '*' in chromosome[position-1]) and len(chromosome[position-1])>1:
               ...
               # Create a record to keep track of the mutation operation
                operation = {'Mut_Type': mutation_type,'Chr': chromosome_index + 1,'Pos': position,'Gene': gene_to_delete,'Genome after mutation': source_genome}
