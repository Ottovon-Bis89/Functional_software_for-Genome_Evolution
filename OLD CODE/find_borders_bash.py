    ...
    if mode == "M":
        if duplicated_reads is not None:
            for dup in sorted(duplicated_reads):
                # print(f"dup: {dup}\t{duplicated_reads.get(dup)}\n")
        ...
        for area in breakpoints:
            ...
            positions = []
            for i in area:
                if len(i) < 6 or len(i[2]) < 3:
                    pass
                else:
                    positions.append(int(i[2].split("r")[2]))
...
def find_duplicate_reads(breakpoints):
    ...
    # print(f"grouped_borders: {grouped_borders}")
    # grouped borders:
    ...
    # print(f"connected borders: {connected_borders[-10:]}\n{len(connected_borders)}")
    # print(f"connected_reads: {connected_reads[-10:]}\n{len(connected_reads)}")
    my_list = []

    # print(border_dict.get(('Chr3', 701, 'Chr3', 1401)))


    # for connection in connected_borders:
