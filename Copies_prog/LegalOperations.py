...
class MutationOperations:
    ...
    def mutation_legal_operations(self, source_genome, target_genome):
        ...
        insertion_genome, new_source_genome = self.check_insertion(source_genome, target_genome, new_source_genome)
        deletion_genome = self.check_deletion(target_genome, new_source_genome_with_positions)
        duplication_genome = self.check_duplication(source_genome, target_genome)
    ...
    def modify_source_genome(self, source_genome):
        '''
        This function removes intergenic regions and foreign DNA from the source
        genome, only leaving the genes for comparison with target.
        '''
        new_source_genome = []
        new_source_genome_with_positions = []
    ...
    def check_insertion(self, source_genome, target_genome, new_source_genome):
        # Initialize empty list to store the insertion mutations required
        insertion_genome = []

        # Iterate over the chromosomes in target
        for j in range(len(target_genome)):
            insertion_target = []

            # Check if target and source has the same number of chromosomes
            # if the length of target <= length of new source then get the chromosomes
            # from both source and target at the same positions and compare for differences
            if j < len(new_source_genome):
                target_chromosome = target_genome[j]
                new_chromosome = new_source_genome[j]
            # else:
            ...
            # Iterate over the genes in target chromosomes and check if target genes are in the isolated source chromosome
            for i in range(len(target_chromosome)):
                if ('*' not in str(target_chromosome[i])) and ((target_chromosome[i]) not in new_chromosome):
            ...
            # Add the required insertion mutation to the list of insertion mutations
            insertion_genome.append(insertion_target)

        # If length of target greater than source, copy over the extra chromosome from target to source
        if len(target_genome) > len(source_genome):
            for missing_chromosome in range(len(source_genome), len(target_genome)):
                new_chromosome = target_genome[missing_chromosome]
                new_source_genome.append(new_chromosome)
        ...
        # Return the list of required insertion mutations for source and the modified source (If the length was < target else return original source genome)
    ...
    def check_deletion(self, target_genome, new_source_genome_with_positions):
        ...
        if len(target_genome) > len(new_source_genome_with_positions):
            for gene_target in range(len(new_source_genome_with_positions), len(target_genome)):
                target_chromosome = target_genome[gene_target]
                deletion_target = ([(position, gene) for position, gene in enumerate(target_chromosome)])
                deletion_genome.append(deletion_target)

        return deletion_genome

    def check_duplication(self, source_genome, target_genome):
        new_source_genome = self.modify_source_genome(source_genome)
        duplication_genome = []
        duplicated_genes = []
